...
# Параметров: 110 (vs 146 с kernel=5)
# Receptive field: 5 отсчётов (vs 7 с kernel=5)
class CNN_DPD_Micro(nn.Module):
    def __init__(self):
        super().__init__()
        # k=3 — меньший receptive field, меньше риска ISI при слабой нелинейности
...
def inference_cnn(signal, batch_size, model, device, weights_file, seq_len=128):
    ...
    # Overlap-add: receptive field = (k1-1)/2 + (k3-1)/2 = 1+1 = 2, берём 4 с запасом
    overlap = 4
