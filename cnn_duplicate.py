...
# Параметров: 146 (vs 74 у оригинала, vs 160 у KAN)
# Receptive field: 7 отсчётов (vs 5 у оригинала)
class CNN_DPD_Micro(nn.Module):
    def __init__(self):
        super().__init__()
        # k=5 на первом слое — захватываем больший временной контекст
        self.conv1 = nn.Conv1d(in_channels=2, out_channels=6, kernel_size=5, padding=2)
        self.act1 = nn.Tanh()

        # Pointwise: перемешивание I/Q каналов
    ...
    def forward(self, x):
        ...
        delta = self.conv3(feat)       # предсказываем ПОПРАВКУ
        out = x_t + delta              # residual: выход = вход + поправка
...
def data_prepare_cnn(objects, targets, seq_len=1000, batch_size=64):
...
def inference_cnn(signal, batch_size, model, device, weights_file, seq_len=1000):
    ...
    # Overlap-add инференс: overlap = (k1-1)/2 + (k3-1)/2 = 2 + 1 = 3, берём с запасом = 4
    overlap = 4
    ...
    # Хвост не покрытый блоками — pass-through
...
def main(train_en=1, lsb=2, mod_order=64):
    ...
    print("Показываем первые 200 точек ДО обучения...")
    ...
    train_dataloader, test_dataloader = data_prepare_cnn(objects, targets, seq_len=seq_len, batch_size=batch_size)
    ...
    if train_en:
        model, train_loss_avg_arr, test_loss_avg_arr = train_cnn(
            model, train_dataloader, test_dataloader, criterion, criterion, num_epochs, optimizer, scheduler, DEVICE
        )
        ...
        plt.grid()
        plt.legend()
        plt.title("Train and Test Loss (CNN)")
        plt.show()
    ...
    avg_test_loss, preds_flat, targets_flat, objects_flat = test_cnn(model, test_dataloader, criterion_test)
    print(f"MAE on test set is {avg_test_loss}")
