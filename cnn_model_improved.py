...
class CNN_DPD_Micro(nn.Module):
    def __init__(self):
        ...
        self.conv2 = nn.Conv1d(in_channels=8, out_channels=8, kernel_size=1, padding=0)
        self.act2  = nn.Tanh()

        self.conv3 = nn.Conv1d(in_channels=8, out_channels=7, kernel_size=1, padding=0)
        self.act3  = nn.Tanh()

        self.conv4 = nn.Conv1d(in_channels=7, out_channels=2, kernel_size=3, padding=1)

    def forward(self, x, return_delta=False):
        x_t   = x.transpose(1, 2)
        feat  = self.act1(self.conv1(x_t))
        feat  = self.act2(self.conv2(feat))
        feat  = self.act3(self.conv3(feat))
        delta = self.conv4(feat)
        out   = x_t + delta
        if return_delta:
...
def inference_cnn(signal, batch_size, model, device, weights_file, seq_len=128,
                  zero_conv3=False):
    """
    zero_conv3=True  — диагностический режим: обнуляет conv3 после загрузки весов,
                       модель вернёт вход без изменений (identity).
                       Если penalty при этом улучшается — CNN вносит вредные артефакты.
    zero_conv3=False — обычный inference с загруженными весами.
    """
    ...
    if zero_conv3:
        ...
        print("[inference_cnn] Diagnostic mode: conv3 zeroed (identity mapping)")
...
def main(train_en=1, lsb=2, mod_order=64):
    ...
    plot_signals_200(objects_flat, targets_flat, preds=preds_flat,
                     title="Test Set Results (After Training)")
