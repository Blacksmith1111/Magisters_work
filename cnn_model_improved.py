...
class CNN_DPD_Micro(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv1d(2, 6, kernel_size=3, padding=1)
        self.act1 = nn.Tanh()
        ...
        self.conv4 = nn.Conv1d(3, 2, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        feat = self.act3(self.conv3(feat))
        delta = self.conv4(feat)
        return (x_t + delta).transpose(1, 2)
...
def inference_cnn(signal, batch_size, model, device, weights_file, seq_len=128, zero_conv3=False):
